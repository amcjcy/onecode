...
while :; do
  ...
  case "$1" in
	--is_auto)
      is_auto=y; shift 1
      [ -d "/code/onecode" ] && { echo "Code 后端 已经存在"; exit 1; }
      ;;
    --is_mu)
      is_mu=y; shift 1
      ;;
...
echo "检查是否已安装 Code 后端..."

if [ -d "/code/onecode" ]; then
	while :; do echo
		echo -n "服务器已安装 Code 后端！如果继续安装，之前的所有配置都将丢失！?(Y/N)"
		read is_clean_old
		if [[ ${is_clean_old} != "y" && ${is_clean_old} != "Y" && ${is_clean_old} != "N" && ${is_clean_old} != "n" ]]; then
			...
			rm -rf /code
			rm -rf /etc/systemd/system/code.service
			break
		else
			exit 0
		fi
...
do_service(){
	echo "等待系统配置..."
	cp -r /code/onecode/code.service /etc/systemd/system/
	echo "正在启动 Code 后端..."
	systemctl daemon-reload && systemctl enable code && systemctl start code
}
while :; do echo
	echo -n "是否要将 Code 后端 加入开机自动启动?(Y/N)"
	read is_service
